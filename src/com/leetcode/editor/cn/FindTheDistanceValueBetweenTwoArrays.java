...
// 0 <= d <= 100 
// 
// Related Topics 数组 双指针 二分查找 排序 👍 23 👎 0
...
class Solution {
	...
	public int findTheDistanceValue2(int[] arr1, int[] arr2, int d) {
		...
		for (int i : arr1) {
			if (i<min){
				...
				if (Arrays.stream(arr2).allMatch(f->Math.abs(f-i)>d)){
					count++;
				}
			}
		}
	...
	public int findTheDistanceValue(int[] arr1, int[] arr2, int d) {
		return (int) Arrays.stream(arr1).filter(i -> Arrays.stream(arr2).allMatch(f->Math.abs(f-i)>d)).count();
